...
class ObjectDetection(object):
    ...
    def __init__(
        ...
        self.serve = model.signatures['serving_default']  # type: ignore

    def _logistic(self, x):
        return np.where(x > 0, 1 / (1 + np.exp(-x)), np.exp(x) / (1 + np.exp(x)))

    def NMS(self, boxes, class_probs, max_detections):
        ...
        while len(selected_boxes) < max_detections:
            ...
            max_probs[overlapping_indices] = np.amax(
                class_probs[overlapping_indices],
                axis=1
            )
    ...
    def preprocess(self, image):
        ...
        cv.resize(image, (new_width, new_height))
        return image

    def predict(self, preprocessed_inputs):
        inputs = np.array(
            preprocessed_inputs, dtype=np.float32
        )[np.newaxis, :, :, (2, 1, 0)]  # type: ignore
    ...
    def postprocess(self, prediction_outputs):
        ...
        selected_boxes, selected_classes, selected_probs = self.NMS(
            boxes[index],
            class_probs[index],
            self.max_detections
        )
